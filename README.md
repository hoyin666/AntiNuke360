...
AntiNuke360 是一個強大的 Discord 伺服器防護機器人，專門針對核打擊 (Nuke) 攻擊進行防禦。它提供多層防護機制，包括實時攻擊偵測、自動封鎖惡意用戶、全域黑名單系統、伺服器快照自動還原，以及反被盜帳偵測功能。
...
- **每小時定期檢查一次 Administrator 權限** (v1.2.2 增強)
- 若無權限，通知擁有者和管理員後自動離開
- 確保機器人持續具備完整的防護能力
...
- 永久白名單成員不會因為可疑訊息被踢出
- **增強 Administrator 權限檢查：現在每小時定期檢查一次 Administrator 權限**
- 增強安全性：更完善的帳號安全保護
